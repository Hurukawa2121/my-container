...
int capabilities() {
  ...
  /**
   * 一部のcapabilityは namespace 内で完結するなら比較的安全
   * ・ CAP_NET_ADMIN：「ネットワーク名前空間」と組み合わせて、ホストのネットワークではなくコンテナ側の仮想デバイスにしか影響を与えない
   * ・ CAP_IPC_OWNER：「IPC名前空間」内だけならホストに影響が出ないため、残しても問題ないことが多い
   */
...
int syscalls() {
	...
	scmp_filter_ctx filter_context = prohibit_syscalls();
	// ?prohibit_syscallsに入れても残るかも？seccomp_release(filter_context);
  if (filter_context) {
		fprintf(stderr, "failed: %m\n");
		return 1;
	}
...
#define SHARES "256" // cpu
#define PIDS "64"
#define WEIGHT "10" // blkio
#define FD_COUNT 64
...
int choose_hostname(char *buff, size_t len) {
	const char *suits[] = { "swords", "wands", "pentacles", "cups" };
	const char *minor[] = {
